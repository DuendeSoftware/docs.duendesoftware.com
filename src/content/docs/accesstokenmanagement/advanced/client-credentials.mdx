...
<Tabs syncKey="atm">
  ...
  <TabItem label="V4">
    ...
    ```csharp
    //program.cs
    services.AddSingleton<HybridCache>(new MyCustomCacheImplementation());
    ```

    Or only for `Duende.AccessTokenManagement`, by using Service Keys:
    ```csharp
    //program.cs
    services.AddSingleton<HybridCache>(ServiceProviderKeys.ClientCredentialsTokenCache, new MyCustomCacheImplementation());
    ```
