...
* **Step 1**: IdentityServer exposes a [backchannel authentication request endpoint](../reference/endpoints/ciba) that the client uses to initiate the CIBA workflow.
...
This is done by using the [IBackchannelAuthenticationUserValidator](../reference/validators/ciba_user_validator) service in DI, **which you are required to implement and register in the DI system**.
...
* **Step 3**: Once a user has successfully been identified, then a record representing the pending login request is created in the [Backchannel Authentication Request Store](../reference/stores/backchannel_auth_request_store).

* **Step 4**: Next, the user needs to be notified of the login request. This is done by using the [IBackchannelAuthenticationUserNotificationService](../reference/services/ciba_user_notification) service in DI, **which you are required to implement and register in the DI system**.
...
This method is passed a [BackchannelUserLoginRequest](../reference/models/ciba_login_request) which will contain all the contextual information needed to send to the user (the *InternalId* being the identifier for this login request which is needed when completing the request -- see below).
...
The [IBackchannelAuthenticationInteractionService](../reference/services/ciba_interaction_service) can be used to access an indivdual [BackchannelUserLoginRequest](../reference/models/ciba_login_request) by its *InternalId*. Once the user has consented and allows the login, then the *CompleteLoginRequestAsync* method should be used to record the result (including which scopes the user has granted).
...
:::note
We provide [a sample](../samples/misc#client-initiated-backchannel-login-ciba) for the interactive pages a user might be presented with for the CIBA workflow.
:::
