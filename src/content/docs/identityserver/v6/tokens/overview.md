...
OIDC and OAuth contain two endpoints that can issue tokens - the [authorize endpoint](/identityserver/v6/reference/endpoints/authorize) and the [token endpoint](/identityserver/v6/reference/endpoints/token).
...
### Access Token
An access token is a data structure that allows a client to access a resource (e.g. an API - see the [protecting APIs](/identityserver/v6/apis) section for more details).
...
:::note
You can control the access token format on a per-client basis using the [AccessTokenType](/identityserver/v6/reference/models/client#token) setting.
:::
