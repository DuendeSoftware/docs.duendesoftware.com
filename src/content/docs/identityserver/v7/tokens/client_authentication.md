...
:::note
All information in this section also applies to [API secrets](../reference/models/api_resource) for introspection.
:::
...
It is the job of implementations of the [ISecretParser](../reference/models/secrets#duendeidentityservervalidationisecretparser) interface to accomplish this. You can add secret parsers by calling the `AddSecretParser()` DI extension method.
...
### Secret validation
It is the job of implementations of the [ISecretValidator](../reference/models/secrets.md#duendeidentityservermodelparsedsecret) interface to validate the extracted credentials.
...
See [here](../samples/basics#jwt-based-client-authentication) for a sample for using JWT-based
authentication.
...
See [here](../samples/basics#mvc-client-with-jar-and-jwt-based-authentication) for a sample for using
...
*`StrictClientAssertionAudienceValidation`*](../reference/options#strict-audience-validation)
...
If `typ` is not
present, [default audience validation](../apis/aspnetcore/jwt#adding-audience-validation)
is used.
...
Use the [DI extensions methods](../reference/di) to add the services to DI which contain a default
