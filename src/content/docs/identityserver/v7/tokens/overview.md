...
OIDC and OAuth contain two endpoints that can issue tokens - the [authorize endpoint](../reference/endpoints/authorize) and the [token endpoint](../reference/endpoints/token).
...
### Access Token
An access token is a data structure that allows a client to access a resource (e.g. an API - see the [protecting APIs](../apis) section for more details).
...
:::note
You can control the access token format on a per-client basis using the [AccessTokenType](../reference/models/client#token) setting.
:::
