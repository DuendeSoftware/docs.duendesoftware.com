...
This is done automatically if you [authenticate](../tokens/client_authentication#mutual-tls-client-certificates) the client using a TLS client certificate.
...
can [validate](../apis/aspnetcore/confirmation) the `cnf` claim by comparing it to the thumbprint of the
...
There are additional client as well as [global](../reference/options#dpop) DPoP settings to control the
behavior.
...
See [here](../apis/aspnetcore/confirmation#validating-dpop-proof-of-possession) for documentation
