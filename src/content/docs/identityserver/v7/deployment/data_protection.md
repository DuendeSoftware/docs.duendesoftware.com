...
Separately, IdentityServer needs cryptographic keys, called [signing keys](/identityserver/v7/fundamentals/keys), to
...
through [discovery](/identityserver/v7/reference/endpoints/discovery). The private key component of the signing keys are
...
* protecting signing keys at rest (
  if [automatic key management](/identityserver/v7/fundamentals/keys/automatic_key_management) is used and enabled)
