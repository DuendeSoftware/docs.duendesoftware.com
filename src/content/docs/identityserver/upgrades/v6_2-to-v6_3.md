...
* [Support](/identityserver/reference/endpoints/authorize.md#optional-parameters) for the OIDC `prompt=create` parameter, which gives the client application the ability to provide a hint that the user needs to register. 
...
IdentityServer 6.3 adds new four new properties to the [`Duende.IdentityServer.Models.Client` model](/identityserver/reference/models/client.md) that are needed to support DPoP and 3rd party initiated login. If you are storing you Client configuration in a [database](/identityserver/data), then you will need to update the database's schema. 
