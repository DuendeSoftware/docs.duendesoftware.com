...
_`a)`_ Add the authentication/authorization related package to the client project file:
...
_`b)`_ Add a using statement to `_Imports.razor` to bring the above package in scope:

```cs
@using Microsoft.AspNetCore.Components.Authorization
```

_`c)`_ To propagate the current authentication state to all pages in your Blazor client, you add a special component
...
see [here](/bff/fundamentals/session/management/user.md)). We will now add a Blazor `AuthenticationStateProvider` that
...
See [here](/bff/fundamentals/apis/local.mdx) for more background information.
...
Our Blazor BFF [sample](/bff/samples/index.mdx#blazor-wasm) is based on this Quickstart. In addition it shows
concepts like
