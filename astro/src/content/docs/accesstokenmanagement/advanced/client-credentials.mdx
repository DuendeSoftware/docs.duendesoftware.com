...
* `UseCacheAutoTuning` - When enabled, the cache uses the actual token expiration time to set the cache entry lifetime. The first request uses `DefaultCacheLifetime` since the expiration isn't known yet. Defaults to `true`.
