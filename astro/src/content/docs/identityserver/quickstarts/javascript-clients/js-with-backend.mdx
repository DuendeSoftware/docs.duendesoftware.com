...
from a copy of the [reference implementation of Quickstart 3](https://github.com/DuendeSoftware/Samples/tree/main/IdentityServer/v8/Quickstarts/3_AspNetCoreAndApis).
...
WebClient did in the prior [web application quickstart](/identityserver/quickstarts/3-api-access.md). Additionally, the BFF services need to be added with
...
extension method on the `HttpContext`. For example: _var token = await
HttpContext.GetUserAccessTokenAsync();_
:::
...
Notice both API calls require an _'X-CSRF': '1'_ header, which acts as the
anti-forgery token.

:::note
See the [client credentials quickstart](/identityserver/quickstarts/1-client-credentials.md) for information on how to create the
