...
description: "Learn how to protect an MCP Server with Duende IdentityServer and connect an MCP client using OAuth and Dynamic Client Registration."
...
AI agents and tools increasingly communicate via the Model Context Protocol (MCP). This quickstart secures an MCP server with Duende IdentityServer and connects a client using [Dynamic Client Registration (DCR)](/identityserver/configuration/dcr.mdx). The client registers itself, so you do not need to configure it in advance.

:::note
This quickstart uses DCR with IdentityServer v8. See [AI & MCP](/identityserver/ai/index.mdx) for more registration guidance and other client-registration options.
:::

## Quickstart Applications

We'll create three projects: an IdentityServer host, an MCP server, and a console client. The client calls the MCP server but signs in through IdentityServer.
...
```
#### Map DCR in the IdentityServer Pipeline
...
#### Update Launch Url For Local Development
...
```json {9} title="~/mcp-quickstart/McpQuickStart.IdentityServer/Properties/launchSettings.json"
{
  "profiles": {
    "https": {
      "commandName": "Project",
...
```json {7} title="~/mcp-quickstart/McpQuickStart.McpServer/Properties/launchSettings.json"
{
  "profiles": {
    "https": {
      "commandName": "Project",
...
Opening browser to: https://localhost:5001/connect/authorize?client_id=AZwWIaA8ApNcB5jptVQrSaNO4hF-nbRtBz19QnKEGOI&redirect_uri=http%3a%2f%2flocalhost%3a1179%2fcallback&response_type=code&code_challenge=mfZCS1wY7EHZwUkIb50SD6dmzReczXjyDMC_GFKEHvM&code_challenge_method=S256&state=EAx8LsLDnK0gVNY8Oxw8wF0Jv6TPCu9-YlyHL7XfFHE&resource=https%3a%2f%2flocalhost%3a7141&scope=mcp%3atools+offline_access
Listening for OAuth callback on: http://localhost:1179/
