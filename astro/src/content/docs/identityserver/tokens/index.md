...
OIDC and OAuth contain two endpoints that can issue tokens - the [authorize endpoint](/identityserver/reference/v8/endpoints/authorize.md) and the [token endpoint](/identityserver/reference/v8/endpoints/token.md).
...
:::note
You can control the access token format on a per-client basis using the [AccessTokenType](/identityserver/reference/v8/models/client.md#token) setting.
:::
