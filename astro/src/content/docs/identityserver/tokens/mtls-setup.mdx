...
## ASP.NET Core OpenID Connect Clients

An interactive ASP.NET Core web application uses mTLS for the same two reasons as any other client: to authenticate the client with a certificate instead of a shared secret, and/or to obtain [certificate-bound (sender-constrained) access tokens](/identityserver/tokens/pop.md). What differs is how you wire it up.

The [.NET client examples](/identityserver/tokens/client-authentication.md#net-client-library-2) use [Duende IdentityModel](/identitymodel/index.mdx) to request tokens directly, and they read the mTLS token endpoint from `disco.MtlsEndpointAliases` themselves. An interactive web application is different: the OpenID Connect handler makes its **back-channel** calls for you. It handles authorization code redemption and, on .NET 9 and later, the [pushed authorization request](/identityserver/tokens/par.md). To use mTLS from such a client, you need to do two things:
...
### Applying the mTLS Endpoint Aliases
...
<LinkCard
  ...
  description="Register mTLS client secrets (certificate name or thumbprint) and compare client secrets, private key JWT, and mutual TLS."
  href="/identityserver/tokens/client-authentication.md#mutual-tls-client-certificates"
/>
