...
Configuration and operational storage are registered together by a single call, and they share the configured database
provider and schema. To use a different provider for one specific store, call an explicit store registration such as
`AddInMemoryClients()` or `AddInMemoryPersistedGrants()` afterward; it replaces only that store's Duende Storage-backed
registration and leaves the rest of the Duende Storage wiring in place. You can also combine Duende Storage with another
provider this way.
