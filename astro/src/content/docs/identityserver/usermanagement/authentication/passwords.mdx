...
`PlainTextPassword` is a validated value type that cannot be constructed directly. When you need to create or change a password, use `IUserAuthenticatorsSelfService`, which applies the full set of validation rules configured in [`PasswordOptions`](#passwordoptions) before returning a value. These async creation methods are available:
...
public interface IUserAuthenticatorsSelfService
{
    ...
    Task<PlainTextPassword> CreatePasswordAsync(string passwordString, CancellationToken ct);

    // Returns a PasswordCreationResult indicating success or failure.
    Task<PasswordCreationResult> TryCreatePasswordAsync(string passwordString, CancellationToken ct);
...
* Use `IUserAuthenticatorsSelfService.CreatePasswordAsync()` or `TryCreatePasswordAsync()` when the user is **setting or changing** a password. Full validation runs, including length, character class requirements, and any registered `IPasswordValidator` implementations.
...
`UserName` is a strongly-typed value representing the user's login identifier (email address, username, or other unique identifier). Use `Create` when you expect valid input, or `TryCreate` in user-facing flows where you want to handle invalid input gracefully:

```csharp
public record UserName
{
    public static UserName Create(string value);
    public static bool TryCreate(string? input, [NotNullWhen(true)] out UserName? result);
}
```
    ...
    CancellationToken ct)
{
    ...
    var creationResult = await authenticatorsSelfService.TryCreatePasswordAsync(newPassword, ct);
...
See [TOTP Authentication Flow](/identityserver/usermanagement/authentication/totp.mdx) for the second-factor implementation, [Passkeys as Second Factor](/identityserver/usermanagement/authentication/passkeys#second-factor-passkey-authentication) for using passkeys as a second step, and [Recovery Codes](/identityserver/usermanagement/authentication/recovery-codes) for backup access.
...
For full details on the `IPasswordHashAlgorithm` interface, `HashedPasswordData`, transparent re-hashing via `NeedsRehash()`, the built-in PBKDF2-SHA-512 algorithm, and how to implement custom algorithms, see the [Password Hashing Algorithms](/identityserver/usermanagement/reference/password-hashing.md) reference.
