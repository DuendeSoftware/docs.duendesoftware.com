...
`PlainTextPassword` is a validated value type that cannot be constructed directly. To create one, use the factory methods on `IUserAuthenticatorsSelfService`. These methods validate the password string against the full set of rules configured in [`PasswordOptions`](#passwordoptions) and return a `PlainTextPassword` on success. They do not store the password or modify the user's account. You pass the resulting `PlainTextPassword` to a lifecycle method like `TrySetPasswordAsync` or `TryChangePasswordAsync` to actually persist it. Both factory methods take the `UserSubjectId` of the user setting the password as their first argument, so custom validators can apply per-user policy if needed:
...
public interface IUserAuthenticatorsSelfService
{
    ...
    // Validates the password string and returns a PlainTextPassword. Does NOT set the password.
...
* Use `IUserAuthenticatorsSelfService.CreatePasswordAsync(userId, ...)` or `TryCreatePasswordAsync(userId, ...)` when the user is **setting or changing** a password. These are factory methods that validate the input and return a `PlainTextPassword` object. They do not persist anything. Pass the result to a lifecycle method like `TrySetPasswordAsync` or `TryChangePasswordAsync` to store it.
