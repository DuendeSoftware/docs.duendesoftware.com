...
{
    // Create a new user with an external authenticator
    Task<UserAuthenticators?> TryCreateAsync(
...
public sealed record ExternalAuthenticatorAddress(
    ExternalAuthenticatorAddressName Name, // Provider name (e.g., "Google")
    SubjectId SubjectId                    // Provider's subject identifier
);
```

`ExternalAuthenticatorAddressName` - Identifies the external provider:

```csharp
public record ExternalAuthenticatorAddressName
{
    // Typically matches the ASP.NET Core authentication scheme name
    public static ExternalAuthenticatorAddressName Create(string input);
    public static bool TryCreate(string? input, [NotNullWhen(true)] out ExternalAuthenticatorAddressName? result);
}
```
...
public static async Task HandleExternalAuthentication(TicketReceivedContext context)
{
    ...
    var externalAuthenticatorName = ExternalAuthenticatorAddressName.Create(context.Scheme.Name);
...
public static async Task HandleAddAuthenticator(TicketReceivedContext context)
{
    ...
    var providerName = ExternalAuthenticatorAddressName.Create(context.Scheme.Name);
    var providerSubject = OpaqueSubjectId.Create(
        context.Principal.FindFirst(JwtClaimTypes.Subject)!.Value);

    var authenticator = new ExternalAuthenticatorAddress(providerName, providerSubject);
    ...
    CancellationToken ct)
{
    ...
    var authenticator = new ExternalAuthenticatorAddress(
        ExternalAuthenticatorAddressName.Create(providerName),
