...
`TryAddOtpAddressAsync` verifies the OTP before persisting the address. The caller must first send an OTP using `IOtpSender.TrySendOtpAsync`, then pass the OTP code and token back for verification. This ensures the user actually controls the address being added.
...
if (sendResult is null || !sendResult.Sent)
    return Error("Failed to send verification code.");

// Store sendResult.Token for the verification step (e.g., in a cookie or session)
