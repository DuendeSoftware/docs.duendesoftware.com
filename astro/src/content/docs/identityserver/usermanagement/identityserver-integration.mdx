---
title: "Enabling User Management in IdentityServer"
description: "How to add User Management to IdentityServer, including user storage, authentication flows, claims mapping, and role-based authorization."
date: 2025-05-26
sidebar:
  label: IdentityServer Integration
  order: 3
---

import { Steps } from "@astrojs/starlight/components";

The `Duende.IdentityServer.UserManagement` package is what you add to your IdentityServer project to enable User Management.
It wires up user storage, authentication flows, and claims mapping through a single `AddUserManagement()` call on the IdentityServer builder.
Under the hood, it registers an `IProfileService` implementation that maps user profile attributes to OIDC claims and emits role claims
(both direct and transitive, deduplicated).

## Setup

Once you have IdentityServer set up, adding User Management takes a single call on the builder.

<Steps>

1. **Add the NuGet package**

    ```bash title="Terminal"
    dotnet add package Duende.IdentityServer.UserManagement
    ```

2. **Call `AddUserManagement()` on the IdentityServer builder**

    The configure delegate is where you enable features and configure storage. This registers both the User Management
    services and the `UserManagementProfileService` implementation of `IProfileService`.

    ```csharp
    // Program.cs
    builder.Services
        .AddIdentityServer(options =>
        {
            options.UserInteraction.LoginUrl = "/Account/Login";
            options.UserInteraction.LogoutUrl = "/Account/Logout";
        })
        .AddInMemoryIdentityResources(Config.IdentityResources)
        .AddInMemoryApiScopes(Config.ApiScopes)
        .AddInMemoryClients(Config.Clients)
        .AddUserManagement(options =>
        {
            options.EnableAuthentication(configure =>
            {
                configure.UseSmtpOtpSender(x => builder.Configuration.GetSection("Smtp").Bind(x));
            });
            options.EnableProfiles();
            options.AddPostgreSqlStore();
        });
    ```

    :::tip
    You do not need to call `AddUserManagement()` separately on `IServiceCollection`.
    The extension method on `IIdentityServerBuilder` handles both registrations.
    :::
</Steps>

## Claims Mapping

When IdentityServer calls `GetProfileDataAsync`, the `UserManagementProfileService` loads the user's profile
and maps it to claims:

* **Profile attributes**: Each attribute's code becomes the claim type and its value becomes the claim value. Boolean attributes emit `"true"` or `"false"` with `ClaimValueTypes.Boolean`.
* **`preferred_username`**: Emitted from the user's `UserName` property.
* **`role`**: Emitted for every role assigned directly to the user and for every role inherited transitively through group membership. Duplicate roles (by role ID) are deduplicated before emission.

Only claims that match the requested scopes are issued. The `UserManagementProfileService`
uses `context.AddRequestedClaims(claims)` internally, so IdentityServer's standard scope-to-claim filtering applies.

## Partial Module Registration

You don't have to enable every User Management module. The `UserManagementProfileService` handles missing modules gracefully:

* **Profiles module not registered** (no `IUserProfileAdmin` in the service provider): The profile service falls back to passing through the claims from the authentication session's subject principal. `IsActiveAsync` treats the user as always active. This matches the behavior of IdentityServer's built-in `DefaultProfileService`.
* **Membership module not registered** (no `IMembershipAdmin` in the service provider): Role claim resolution is skipped entirely. No `role` claims are emitted.

This means you can call `AddUserManagement()` with only authentication enabled, for example, and the profile service still works without errors.

## Subject ID Requirements

The `sub` claim in the token must be a non-empty string (up to 200 characters) that can be parsed as a `UserSubjectId`.
The `UserManagementProfileService` converts it using `UserSubjectId.TryCreate`. If parsing fails, no claims are issued and a warning
is logged.

`IsActiveAsync` returns `true` when the user profile exists and `false` otherwise, which causes IdentityServer to
reject tokens for deleted or missing users.

## Customization

`UserManagementProfileService` is a public class with virtual methods you can override by subclassing it and registering your
subclass with the service provider:

* `FindUserAsync(UserSubjectId subjectId, CancellationToken ct)` - override to customize how the user profile is loaded.
* `GetProfileDataAsync(ProfileDataRequestContext context, UserSubjectId subjectId, CancellationToken ct)` - override to intercept profile loading after the subject ID is parsed but before the user profile is fetched.
* `GetProfileDataAsync(ProfileDataRequestContext context, UserProfile user, CancellationToken ct)` - override to customize how claims are built from the profile.
* `GetRoleClaimsAsync(ProfileDataRequestContext context, UserSubjectId subjectId, CancellationToken ct)` - override to customize how role claims are collected.

Register your subclass after calling `AddUserManagement()`:

```csharp
// Program.cs
builder.Services
    .AddIdentityServer(...)
    .AddUserManagement(options => { ... })
    .AddProfileService<CustomProfileService>();
```
