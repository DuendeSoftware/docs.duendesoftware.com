...
redirect_from:
  ...
  - /identityserver/reference/services/ciba-interaction-service/
---
...
* **`GetPendingLoginRequestsForCurrentUserAsync(CancellationToken ct)`**

  Returns an `IReadOnlyCollection<BackchannelUserLoginRequest>` representing pending login requests for the current user.

* **`GetLoginRequestByInternalIdAsync(string id, CancellationToken ct)`**

  Returns the [BackchannelUserLoginRequest](/identityserver/reference/v8/models/ciba-login-request.md) object for the id.
