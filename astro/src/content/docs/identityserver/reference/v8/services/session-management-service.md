...
redirect_from:
  ...
  - /identityserver/reference/services/session-management-service/
---

#### Duende.IdentityServer.Services.ISessionManagementService

When using [server-side sessions](/identityserver/ui/server-side-sessions/index.md), the `ISessionManagementService` provides
...
public interface ISessionManagementService
{
    ...
    Task<QueryResult<UserSession>> QuerySessionsAsync(SessionQuery? filter, CancellationToken ct);
    ...
    /// <param name="ct">The cancellation token.</param>
    Task RemoveSessionsAsync(RemoveSessionsContext context, CancellationToken ct);
}
```
