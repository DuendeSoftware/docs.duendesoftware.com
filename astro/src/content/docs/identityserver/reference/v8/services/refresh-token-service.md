...
redirect_from:
  ...
  - /identityserver/reference/services/refresh-token-service/
---
...
:::note[Breaking change in v8.0]
`CreateRefreshTokenAsync` and `UpdateRefreshTokenAsync` signatures changed in v8.0. The individual parameters
(`ClaimsPrincipal subject`, `Token accessToken`, `Client client`, `string handle`, `RefreshToken refreshToken`) have been
replaced by `RefreshTokenCreationRequest` and `RefreshTokenUpdateRequest` request objects respectively. Update any custom
implementations or subclasses of `DefaultRefreshTokenService` to use the new signatures.
:::
