...
redirect_from:
  ...
  - /identityserver/reference/options/
...
- **`CacheLockTimeout`** *(obsolete)*

  The timeout for concurrency locking in `KeyManager` for key-management operations. This property is no longer used by configuration-store caching (`HybridCache` provides built-in stampede protection). Defaults to 60 seconds.
...
- **`ProofTokenValidityDuration`**
  ...
  Duration that DPoP proof tokens are considered valid. Defaults to _1 minute_.
