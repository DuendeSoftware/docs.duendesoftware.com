...
redirect_from:
  - /identityserver/upgrades/identityserver4-to-duende-identityserver-v7/
---
...
* `ConsentResponse.Denied` was removed. Use `DenyAuthenticationAsync` instead:
...
If you are using the `IdentityServer4.EntityFramework` package to store configuration and operational data in a [database](/identityserver/data/index.mdx),
...
+ <PackageReference Include="Duende.IdentityModel" Version="8.0.2" />
```
...
Whether you are using a [database](/identityserver/data/index.mdx) or a [custom store implementation](/identityserver/reference/v8/stores/index.md) for your configuration and operational data, you'll need to make some changes.
...
- Add new properties to the [`Duende.IdentityServer.Models.Client` model](/identityserver/reference/v8/models/client.md):
...
:::note
This feature is part of the [Duende IdentityServer Standard (add-on), Advanced, Business (legacy) and Enterprise (legacy) Edition](https://duendesoftware.com/products/identityserver).
:::
...
- **`IAuthorizationParametersMessageStore` removed.** This interface and its implementations have been removed. If you used it to pass large authorization parameters, migrate to [Pushed Authorization Requests (PAR)](/identityserver/tokens/par.md).
