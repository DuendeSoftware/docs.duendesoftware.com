...
The deprecated `IAuthorizationParametersMessageStore` interface and its implementations
(`DistributedCacheAuthorizationParametersMessageStore` and `QueryStringAuthorizationParametersMessageStore`)
have been removed.

If your application used this interface to pass large authorization parameters between the authorize endpoint
and the login/consent pages, migrate to [Pushed Authorization Requests (PAR)](/identityserver/tokens/par.md). PAR lets clients send authorization parameters directly to the server before redirecting the user, which avoids URL length limits without needing a server-side message store.
