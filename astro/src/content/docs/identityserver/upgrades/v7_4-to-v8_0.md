...
A new `AddDynamicProvider<THandler, TProviderOptions, TProvider, TConfigureOptions>` builder extension method simplifies registering custom dynamic identity provider types. It consolidates provider type mapping, configure options, and handler DI registration into a single call. See the [dynamic providers documentation](/identityserver/ui/login/dynamicproviders.md#using-non-oidc-authentication-handlers) for details.
