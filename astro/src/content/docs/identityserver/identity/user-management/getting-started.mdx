...
sidebar:
  ...
  order: 2
redirect_from:
  - /identityserver/usermanagement/getting-started/
---
...
<iframe width="853" height="505" src="https://www.youtube.com/embed/8VtPnjE8UJ0" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
For production, switch to PostgreSQL or SQL Server. See the [Storage documentation](/identityserver/identity/user-management/fundamentals/storage.md) for details.
:::
...
This tutorial uses SQLite for simplicity. For production configuration and setup of each provider, see the [Storage documentation](/identityserver/identity/user-management/fundamentals/storage.md).
...
The key call in the code below is `.AddIdentityServer(...).AddUserManagement(...)`: User Management is registered as an extension on the
IdentityServer builder, not as a separate top-level service.

In our application, we can make use of the built-in SMTP dispatcher for sending OTP codes, or use a console-based sender for purpose of this sample.
...
<Tabs syncKey="umgsOtpSender">
  <TabItem label="Console sender (development)">
    ...
    ```csharp title="ConsoleOtpDispatcher.cs"
    ...
    Then open `Program.cs` and replace its contents with the following:
  ...
  <TabItem label="SMTP sender">
    The SMTP sender delivers OTP codes by email. Bind the `Smtp` configuration section to the sender options. Replace the contents of `Program.cs`:
    ...
    builder.Services
        ...
        .AddUserManagement(options =>
        {
            ...
            {
                configure.UseSmtpOtpDispatcher(x =>
...
:::note
You can configure OTP expiry, code length, and rate-limiting behavior through the `OtpAuthenticatorOptions`. See the [OTP authentication docs](/identityserver/identity/user-management/authentication/otp.mdx) for all available options.
:::
...
:::tip
If you need to access the full user record, for example to read additional OTP addresses or linked authenticators, inject `IUserSelfService` and call `Authenticators.TryGetAsync` with the subject ID.
...
<LinkCard
   description="Full source code in Duende User Management samples"
   href="/identityserver/samples/usermanagement#user-management-getting-started-sample"
...
<CardGrid>
  <LinkCard
    ...
    href="/identityserver/identity/user-management/identityserver-integration"
    description="Learn how User Management integrates with IdentityServer for claims mapping and profile services."
  />
  <LinkCard
    title="Storage options"
    href="/identityserver/identity/user-management/fundamentals/storage"
  ...
  <LinkCard
    ...
    href="/identityserver/identity/user-management/authentication/otp"
    description="Configure code length, expiry, rate limiting, and custom OTP senders."
  />
  <LinkCard
    title="User profiles"
    href="/identityserver/identity/user-management/fundamentals/profiles"
    description="Store and retrieve custom attributes for your users."
  />
  <LinkCard
    title="More authenticators"
    href="/identityserver/identity/user-management/authentication/overview"
