...
description: Step-by-step guide to migrating users from an ASP.NET Identity database into Duende User Management using a console app and the IUserImporter API.
sidebar:
...
This guide walks you through building a small console app that reads users from your existing ASP.NET Identity database and imports them into User Management.
The approach is straightforward: query the source database directly, map each user to a `UserImportRecord`, and call `IUserImporter.ImportAsync`.

When the migration is done, your users log in exactly as before. They will not notice anything changed.

Before you start, you need two things: an existing ASP.NET Identity database with users to migrate, and a target application that already has User Management configured
If you have not set up User Management yet, see the [getting started guide](/usermanagement/getting-started.mdx) first.
...
(see [Conflict Resolution](/usermanagement/import/reference.md#conflict-resolution) for details).
...
Start with a new console app:

```bash
dotnet new console -n IdentityMigrator
cd IdentityMigrator
```
...
Replace `Duende.Storage.PostgreSql` and `Npgsql.EntityFrameworkCore.PostgreSQL` with the packages for your database if you are using SQL Server or another provider.
The `Microsoft.AspNetCore.Identity.EntityFrameworkCore` package gives you the `IdentityDbContext` base class, which you will use to read from the source database.

The top-level `Program.cs` uses `HostApplicationBuilder` to wire up services and configuration:
...
// Source: ASP.NET Identity database
...
// AddPostgreSqlStore reads the connection from the NpgsqlDataSource singleton.
var targetConnectionString = builder.Configuration.GetConnectionString("Target")!;
builder.Services
    .AddSingleton(new NpgsqlDataSourceBuilder(targetConnectionString).Build())
...
await RunMigrationAsync(host.Services);
```
...
// appsettings.json
{
  "ConnectionStrings": {
    ...
    "Target": "Host=localhost;Database=usermgmt_db;Username=postgres;Password=..."
  }
}
```

:::note
The example code assumes both source and target are PostgreSQL databases. Other database types can be used as well.
:::
...
See [Password Hashing Algorithms](/usermanagement/reference/password-hashing.md) for a full walkthrough of implementing and registering
...
* **`SubjectId`**: use `UserSubjectId.Parse(user.Id)` to carry the existing GUID across as-is. This preserves any existing tokens or references that use the subject ID.
* **`UserName`**: use `UserName.TryParse(user.UserName, out var parsedUserName)` if the user has a username set.
    ...
    IReadOnlyAttributeSchema schema)
{
    ...
    if (!string.IsNullOrEmpty(user.Email))
    {
        ...
        var emailCode = AttributeCode.Parse("emails");
    ...
    foreach (var claim in claims)
    {
        ...
        var code = AttributeCode.Parse(attrName);
    ...
    {
        var nameCode = AttributeCode.Parse("name");
    ...
    if (!string.IsNullOrEmpty(user.Email) &&
        EmailAddress.TryParse(user.Email, out var emailAddress))
    {
    ...
    if (!string.IsNullOrEmpty(user.UserName) &&
        Duende.UserManagement.UserName.TryParse(user.UserName, out var parsedUserName))
    {
    ...
    {
        SubjectId = UserSubjectId.Parse(user.Id),
        UserName = userName,
...
Now wire everything together. Query users in batches, map each one, and call `ImportAsync`:

```csharp
// Program.cs (continued -- add this method)
static async Task RunMigrationAsync(IServiceProvider services)
{
    await using var scope = services.CreateAsyncScope();
    var sp = scope.ServiceProvider;

    // Ensure the User Management schema exists in the target database.
    await sp.GetRequiredService<IDatabaseSchema>().CreateIfNotExistsAsync(CancellationToken.None);
    ...
    var schema = await profileAdmin.GetSchemaAsync(CancellationToken.None);
    ...
    var claimsByUser = await sourceDb.UserClaims
        .AsNoTracking()
        .GroupBy(c => c.UserId)
        .ToDictionaryAsync(g => g.Key, g => (IReadOnlyList<IdentityUserClaim<string>>)g.ToList());
    ...
    for (var i = 0; i < users.Count; i += batchSize)
    {
        ...
        var result = await importer.ImportAsync(batch, CancellationToken.None);
        ...
        Console.WriteLine(
            $"Batch {i / batchSize + 1}: created={result.CreatedCount}, " +
            $"updated={result.UpdatedCount}, failed={result.FailedCount}");
    }
...
Register it in `Program.cs` before building the host:
...
if (phoneClaim is not null)
{
    ...
    var phoneCode = AttributeCode.Parse("phoneNumbers");
...
Once you are satisfied, remove the `Take(10)` and run the full migration.

**Run against staging first.** Point the `Target` connection string at a staging database and run the full migration there
...
<CardGrid>
  <LinkCard title="Conflict resolution" href="/usermanagement/import/reference#conflict-resolution" description="Control what happens when an imported record collides with existing data." />
  <LinkCard title="Batch processing" href="/usermanagement/import/#batch-processing" description="Chunk large datasets, track progress, and resume after failures." />
  <LinkCard title="Getting started" href="/usermanagement/getting-started" description="Set up User Management in your target application if you have not already." />
</CardGrid>
