...
External authentication delegates sign-in to trusted third-party identity providers using OAuth 2.0 and OpenID Connect. Users authenticate with a provider they already trust (Google, Microsoft, a corporate IdP), and your application receives a verified identity without ever handling a password.
...
| **User Friction** | Click button | Type password | Check email or SMS |
| **Infrastructure** | External identity provider | Password hashing | Email or SMS provider |
...
8. **User provisioning** - Application finds an existing user or creates a new account.
...
```csharp
public readonly record struct ExternalAuthenticatorName
{
    // Typically matches the ASP.NET Core authentication scheme name
    public static ExternalAuthenticatorName Parse(string input);
    public static bool TryParse(string input, out ExternalAuthenticatorName? result);
}
```
...
public static async Task HandleExternalAuthentication(TicketReceivedContext context)
{
    ...
    // Extract provider info from the incoming principal
    var providerName = ExternalAuthenticatorName.Parse(context.Scheme.Name);
    var providerSubject = principal.FindFirst(JwtClaimTypes.Subject)?.Value
        ?? throw new Exception("No subject claim from provider");

    var authenticator = new ExternalAuthenticator(
        providerName,
        SubjectId.Parse(providerSubject));

    // Find or create the user
    var user = await userAuthenticatorsSelfService.TryGetAsync(
        authenticator,
        context.HttpContext.RequestAborted);

    if (user == null)
    {
        // Auto-register: create a new subject ID and register the user
        var newSubjectId = UserSubjectId.New();

        user = await userAuthenticatorsSelfService.TryRegisterAsync(
            newSubjectId,
            authenticator,
            context.HttpContext.RequestAborted);
...
public static async Task HandleAddAuthenticator(TicketReceivedContext context)
{
    ...
    var providerName = ExternalAuthenticatorName.Parse(context.Scheme.Name);
    var providerSubject = SubjectId.Parse(
