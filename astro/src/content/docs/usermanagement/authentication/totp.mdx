...
```csharp
public record TotpAuthenticatorName
{
    // The default authenticator name ("Default")
    public static TotpAuthenticatorName Default { get; }

    // Create an authenticator name, throwing on invalid input
    public static TotpAuthenticatorName Create(string input);

    // Try to parse an authenticator name without throwing
    public static bool TryCreate(string? input, [NotNullWhen(true)] out TotpAuthenticatorName? result);
}
```
