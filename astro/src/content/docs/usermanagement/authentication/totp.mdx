...
public record PlainTextTotp
{
    ...
    public static bool TryParse(string? input, [NotNullWhen(true)] out PlainTextTotp? result);
}
```
...
public record TotpAuthenticatorName
{
    ...
    public static bool TryParse(string? input, [NotNullWhen(true)] out TotpAuthenticatorName? result);
}
```
