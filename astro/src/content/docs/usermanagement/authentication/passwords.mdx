...
When creating an instance of `PlainTextPassword`, a set of validations are run. Validations controlled by [`PasswordOptions`](#passwordoptions) config like minimum/maximum length, minimum amount of symbols, etc. Those validations cannot run when checking a user's password because the validations are run before comparing against the stored value. For example, if the rules have changed since the password was created, like the minimum length moved from 8 characters to 16, then a 12 character password set before the change would never pass validation and thus never get to the stage where the password is compared against the stored value.

For the above reasons, an instance of `NonValidatedPassword` is used when checking a user's password for logging in. When creating an instance of `NonValidatedPassword`, a smaller set of validations are run on the string to verify valid user input (string not null, not empty, etc). Use `TryParse` in user-facing flows where you want to return a validation error rather than catch an exception.
...
```csharp
using Duende.Platform.Users.Authentication.Passwords;
...
builder.Services
    .AddDuendePlatform()
    .AddUserAuthentication(authBuilder =>
    {
        authBuilder.AddPasswordValidator<BlocklistPasswordValidator>();
    })
```
...
It's easy to assume the validations are run when a `PlainTextPassword` instance is created by calling `IPlainTextPasswordFactory.Create()` or `IPlainTextPasswordFactory.TryCreate()`. Those methods verify the password string fits the length/character requirements set by the [configured `PasswordOptions`](#passwordoptions), but it does not call the registered validators.
