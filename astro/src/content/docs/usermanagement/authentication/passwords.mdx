...
public interface IUserAuthenticatorsSelfService
{
    ...
    bool TryCreatePassword(string passwordString, [NotNullWhen(true)] out PlainTextPassword? result);
    
    // Returns false and populates validationErrors if the password does not meet requirements.
    bool TryCreatePassword(string passwordString, [NotNullWhen(true)] out PlainTextPassword? result, [NotNullWhen(false)] out IReadOnlyList<string>? validationErrors);
...
public record NonValidatedPassword
{
    ...
    public static bool TryParse(string? passwordString, [NotNullWhen(true)] out NonValidatedPassword? result);
    public static bool TryParse(string? passwordString, [NotNullWhen(true)] out NonValidatedPassword? result, [NotNullWhen(false)] out IReadOnlyList<string>? errors);
}
```
...
public record UserName
{
    ...
    public static bool TryParse(string? input, [NotNullWhen(true)] out UserName? result);
}
```
