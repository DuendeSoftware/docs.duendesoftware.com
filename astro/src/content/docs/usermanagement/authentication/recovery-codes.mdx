...
<Steps>
1. **Two-Factor Authentication (2FA) activation** - User enables TOTP authentication

2. **Code generation** - The system generates a set of recovery codes (typically 10)

3. **Secure display** - Codes are shown to the user once

4. **User storage** - The user saves codes securely (print, password manager, etc.)

5. **Hashed storage** - Codes are hashed and stored in the database

6. **Availability** - Codes remain available until used or regenerated
</Steps>

When a user enables Two-Factor Authentication (2FA), the system generates a set of recovery codes and shows them once. Here's how that works:
...
<Steps>
1. **2FA required** - User completes primary authentication and needs 2FA

2. **Primary method unavailable** - User cannot access their TOTP authenticator

3. **Recovery option** - User selects "Use recovery code instead"

4. **Code entry** - User enters one of their saved recovery codes

5. **Verification** - System verifies the code against stored hashes

6. **Code consumption** - If valid, the code is marked as used (single-use)

7. **Session establishment** - User is authenticated with a Multi-Factor Authentication (MFA) claim

8. **Warning** - User is notified of remaining codes
</Steps>

When a user can't access their authenticator app, they can sign in with a recovery code instead. The code is verified and immediately consumed:
...
```csharp
public readonly record struct PlainTextRecoveryCode
{
    // Parse a user-supplied string into a recovery code
    public static bool TryParse(string input, out PlainTextRecoveryCode? result);

    // Parse or throw on invalid input
    public static PlainTextRecoveryCode Parse(string input);
...
public async Task<IActionResult> OnPostEnableTotpAsync(string verificationCode)
{
    ...
    var success = await userAuthenticatorsSelfService.TryAddTotpAuthenticatorAsync(
        ...
        totpKey,
        PlainTextTotp.Parse(verificationCode),
        ct);
...
public async Task<IActionResult> OnPostLoginWithRecoveryCodeAsync(string recoveryCode)
{
    ...
    if (!PlainTextRecoveryCode.TryParse(cleanCode, out var code))
    {
    ...
    var success = await recoveryCodeAuth.TryAuthenticateAsync(
        authState.UserId, code.Value, ct);
...
* [TOTP Authentication](/usermanagement/authentication/totp.mdx) - Primary 2FA method that recovery codes back up
* [OTP Authentication](/usermanagement/authentication/otp.mdx) - One-time password authentication
* [Password Authentication](/usermanagement/authentication/passwords.md) - Password-based authentication
