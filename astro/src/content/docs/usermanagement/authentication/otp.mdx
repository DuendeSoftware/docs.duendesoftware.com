...
3. **User lookup or creation** - If the user exists, they are retrieved; otherwise, they can be auto-registered.
...
public interface IOtpAuthenticator
{
    ...
    // Verify an OTP code; returns the OtpAddress on success, null on failure
    Task<OtpAddress?> TryAuthenticateAsync(PlainTextOtp otp, OtpToken token, Ct ct);
}
```

`TrySendOtpAsync` returns a `SendOtpResult` that carries the token and expiry information when the code was sent, or rate-limit information when sending was blocked. `TryAuthenticateAsync` returns the `OtpAddress` on success, or `null` if the code was invalid or expired.
...
```csharp
public readonly record struct OtpChannel
{
...
{
    public static OtpToken Parse(string input);
...
```csharp
public readonly record struct PlainTextOtp
{
    public static PlainTextOtp Parse(string value);
    public static bool TryParse(string input, out PlainTextOtp? result);
}
```
...
OTP delivery requires configuring a sender. Use `UseSmtpOtpSender` on the `IUserAuthenticationBuilder` to configure the built-in SMTP sender:
...
The following example shows a two-step OTP login using Razor Pages. Inject `IOtpAuthenticator` and `IUserAuthenticatorsSelfService` into your page model.
...
public async Task<IActionResult> OnPostVerifyAsync(string code)
{
    ...
    var user = await userAuthenticatorsSelfService.TryGetAsync(address, HttpContext.RequestAborted);
    ...
    if (user == null)
        return Error("Unable to retrieve or create user account");
...
OTP authentication naturally supports auto-registration. After a successful code verification, check whether the user already exists and register them if not:

```csharp
var address = new OtpAddress(OtpChannel.Email, EmailAddress.Parse(email));
...
var phoneAddress = new OtpAddress(OtpChannel.Sms, PhoneNumber.Parse("+15551234567"));
...
var oldAddress = new OtpAddress(OtpChannel.Email, EmailAddress.Parse("old@example.com"));
var newAddress = new OtpAddress(OtpChannel.Email, EmailAddress.Parse("new@example.com"));
...
OTP is a good default for consumer applications — simple, passwordless, and requires no app install. The trade-off is that its security depends entirely on the delivery channel. If someone can read your email or intercept your SMS, they can log in as you.
