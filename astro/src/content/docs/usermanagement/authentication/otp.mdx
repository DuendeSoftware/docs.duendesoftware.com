...
For a comparison of all authentication methods, see [Choosing an Authentication Method](/usermanagement/authentication/overview.mdx#choosing-an-authentication-method).
...
`TrySendOtpAsync` returns a `SendOtpResult` that carries the token and expiry information when the code was sent, or rate-limit information when sending was blocked. `TryAuthenticateAsync` returns an `OtpAuthenticationResult`, which is a discriminated union with two subtypes: `OtpAuthenticationResult.Success` (containing the `Address` and an optional `UserSubjectId`) and `OtpAuthenticationResult.Failure`.
...
public async Task<IActionResult> OnPostVerifyAsync(string code)
{
    ...
    UserAuthenticators? user = null;
    if (otpSuccess.UserSubjectId is { } existingId)
    {
        user = await userAuthenticatorsSelfService.TryGetAsync(existingId, HttpContext.RequestAborted);
    }
...
OTP authentication naturally supports auto-registration. After a successful code verification, use `otpSuccess.UserSubjectId` when present to look up an existing user, and call `TryRegisterAsync` when it is null:
