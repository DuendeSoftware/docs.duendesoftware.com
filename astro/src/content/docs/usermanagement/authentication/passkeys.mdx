...
Using `EnableAuthentication()` on its own:
...
When `EnableAuthentication()` is called, the following HTTP endpoints are registered automatically to handle the WebAuthn ceremony protocol:
...
:::note
Passkey routes (including `/passkeys/js`) are only available when `EnableAuthentication()` has been called during service registration and `app.MapUserManagement()` has been called on the endpoint route builder.
:::
