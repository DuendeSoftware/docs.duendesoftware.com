...
1. **`AddUserAuthentication`** registers the core passkey services: cryptographic signature verification (ES256, ES384, ES512, RS256, RS384, RS512, PS256, PS384, PS512, RS1), WebAuthn registration and authentication ceremonies, credential storage, and challenge management. Use this when you want to drive the passkey flow yourself without the built-in HTTP endpoints.
2. **`EnableUserAuthentication`** registers the web layer on top, including the passkey HTTP endpoints (`/passkeys/register/begin`, `/passkeys/register/complete`, `/passkeys/authenticate/begin`, `/passkeys/authenticate/complete`) and the JavaScript helper (`/passkeys/js`).

Using `AddUserAuthentication` on its own:
...
If you are building a web application and want the ready-to-use passkey endpoints, use `EnableUserAuthentication` instead:

```csharp title="Program.cs"
builder.Services.AddDuendePlatform()
    .AddDuendePlatformWebApp(web =>
    {
        web.EnableUserAuthentication();
    });
```

:::note
`EnableUserAuthentication` internally calls `AddUserAuthentication`, so you do not need to call both.
:::
...
:::note
Passkey routes (including `/passkeys/js`) are only available when `EnableUserAuthentication` has been called and the Duende platform endpoints have been mapped, either via `app.UseIdentityServer()` or `app.MapDuendeEndpoints()`.
:::
...
public class MySecondFactorResolver : ISecondFactorPasskeyAuthenticationResolver
{
    ...
    public async Task<UserSubjectId?> ResolveAsync(CancellationToken ct)
    {
...
public async Task<IActionResult> OnPostLogin(string username, string password, CancellationToken ct)
{
    ...
    if (authenticators?.Passkeys.Count > 0)
    {
        ...
        HttpContext.Session.SetString("PendingUserId", success.UserSubjectId.ToGuid().ToString());
...
TPM (Trusted Platform Module) attestation is the format used by Windows Hello and other platform authenticators backed by a hardware security chip. User Management now validates TPM attestation statements, which means that when `AttestationConveyancePreference` is set to `"direct"` or `"enterprise"`, TPM-attested credentials are fully verified rather than accepted without attestation checks.
...
User Management now includes a verifier for RS1 signatures. This broadens compatibility with older hardware security keys that do not support the preferred ES256 (ECDSA with P-256) or RS256 (RSASSA-PKCS1-v1_5 with SHA-256) algorithms.
...
User Management now automatically removes expired challenges. This keeps the challenge store lean and prevents unbounded growth in long-running applications with high registration or authentication traffic.
...
User Management now uses a constant-time comparison for challenge validation. The comparison always takes the same amount of time regardless of how many bytes match, eliminating this timing side-channel.
...
Register your policy using `WithAttestationTrustPolicy<T>()` on the passkey builder:

```csharp
// Program.cs
builder.AddDuendePlatform()
    .AddUserAuthentication(auth =>
        auth.AddPasskeyAuthentication(passkeys =>
            passkeys.WithAttestationTrustPolicy<MyAttestationTrustPolicy>()));
```
