...
Passkeys provide phishing-resistant, passwordless authentication using the WebAuthn/FIDO2 standardint, face recognition), a device PIN, or a hardware security key. Credentials are cryptographically bound to the origin, so they cannot be used on a different site.
