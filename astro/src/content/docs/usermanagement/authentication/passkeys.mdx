...
1. **`AddUserAuthentication`** registers the core passkey services: cryptographic signature verification (ES256, RS256, RS1), WebAuthn registration and authentication ceremonies, credential storage, and challenge management. Use this when you want to drive the passkey flow yourself without the built-in HTTP endpoints.
