...
In Duende User Management, a *space* is the fundamental tenant-isolation unit. Each space has its own isolated user store, configuration, and authentication settings. This enables multi-tenant deployments where users, roles, and groups in one space are completely separated from those in another.
